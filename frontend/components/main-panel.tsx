  ...
  useEffect(() => {
    const fetchItem = async () => {
      ...
      try {
        ...
        const itemData = await filesAPI.getItemById(selectedId);
        setItem(itemData);

        const pathData = await filesAPI.getItemPath(selectedId);
    ...
    <div className="flex-1 bg-white flex flex-col">
      ...
      <div className="flex-1 overflow-auto">
        {item.type === "folder" ? (
          ...
          <FileContent item={item} onRefresh={onRefresh} onSelectItem={onSelectItem} />
        )}
      </div>
    </div>
  );
